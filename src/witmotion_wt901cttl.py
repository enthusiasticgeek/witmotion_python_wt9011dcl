...
def read_data_packet():
    # Read until two consecutive bytes 0x55 and 0x51 are found
    start_bytes = ser.read(1)
    while start_bytes != b'\x55':
        start_bytes = ser.read(1)

    # Check for the next byte to be 0x51
    if ser.read(1) == b'\x51':
        # Read 18 more bytes after 0x55 and 0x51
        data_packet = start_bytes + b'\x51' + ser.read(18)
